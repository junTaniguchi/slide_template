...
3. 各スライド右上（スライド外）に表示される `SVG出力` ボタンから、その場でSVGをクリップボードへコピーできます。Clipboard API非対応環境では自動的にSVGファイルをダウンロードします。
...
2. 各スライドの `p.lead` には、スライド全体の狙いが一読で伝わるリード文を書き、本編で伝えたいメッセージを可能な限りここへ集約します。特に「本日の合意事項」スライドでは、目的とゴールをリード文に統合し、そこで決定したい範囲がわかるようにしてください。
3. `chart-card` や `viz-card` では、既に入っている絵文字ベースのサンプルを参考にしながら、絵文字・簡易SVG・表・ASCIIアート等で実際の図表／ポンチ絵を描写してください（例: 絵文字タイムライン・シンプルな表組み・インラインSVGなど）。テキストの詳細はリード文にまとめ、図表側では数値推移や構造など視覚的に伝えるべき情報を表現します。灰色の占位枠は用意していないため、Tailwindの `grid` / `flex` / `aspect-*` などのユーティリティを必要に応じて調整し、16:9 の枠内で全要素が読み取れるようサイズと余白を最適化してください。特に背景スライドのように情報量が多い図では、図解が見切れないよう表示領域を広げる指示を必ず記載します。
...
9. アジェンダが切り替わる箇所や内容が飛びそうな箇所では、既存のクッションスライドに補足ストーリーとビジュアル指示を記入し、必要なら補足スライド（appendix）を複製して詳細図解を追加します。章間の橋渡しポンチ絵では、段階間の接続は絵文字矢印（例: ➡️, 🔁）で示し、16:9 のキャンバス内で余白を取りすぎないよう Tailwind の `gap` や `p-*` を微調整するよう明記してください。
10. 文章は自然な日本語で、ビジネス文書らしい簡潔さと具体性を維持してください。
11. すべての出力はHTML全体を返し、既存の構造・クラスや追加説明コメントは変更しないでください。`CONFIDENTIAL` 表示は削除せず維持してください。
12. 各スライドのタイトルは、コメント行に指示内容を残しつつ、HTML要素内には参考例のタイトルを初期値として記載しています。このスタイルを維持し、生成時もコメントは指示、要素内テキストは例として扱ってください。
```
