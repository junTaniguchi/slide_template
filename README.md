...
3. `chart-card` や `viz-card` では、既に入っている絵文字ベースのサンプルを参考にしながら、絵文字・簡易SVG・表・ASCIIアート等で実際の図表／ポンチ絵を描写してください（例: 絵文字タイムライン・シンプルな表組み・インラインSVGなど）。灰色の占位枠は用意していないため、Tailwindの `grid` / `flex` / `aspect-*` などのユーティリティを必要に応じて調整し、16:9 の枠内で全要素が読み取れるようサイズと余白を最適化してください。特に背景スライドのように情報量が多い図では、図解が見切れないよう表示領域を広げる指示を必ず記載します。
