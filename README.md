...
- スライド切り替えボタン（▲▼）とナビゲーションスクリプトはテンプレート動作に必須です。生成AIにコンテンツを流し込む際も、該当HTML/JSを削除・変更しないよう注意してください。
...
3. `chart-card` や `viz-card` では、絵文字・簡易SVG・表・ASCIIアート等を用いて簡易な図表／ポンチ絵を実際に描写してください（例: 絵文字タイムライン・シンプルな表組み・インラインSVGなど）。
4. アイコンや簡易的な図形表現が必要な場合は、Unicode 16.0 / Emoji 16.0 準拠の絵文字で代用可能な箇所を優先的に絵文字で表現してください（例: ✅, 📊, 🚀）。
5. ナビゲーションボタン（`.nav-controls` 内の ▲▼）やSVGコピー機能に関わる HTML・クラス名・データ属性・JavaScript は削除・改変しないでください。文言のみ必要に応じて編集し、ボタンが次スライドへ遷移できる状態を維持します。
